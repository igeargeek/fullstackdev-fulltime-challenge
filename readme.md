...
- Can't select same locker with other user in `Real-time`. (`User A` select unit `#1` on another page then `User B` will see unit `#1` is not available to select without refresh page.)
